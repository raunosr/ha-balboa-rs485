    ...
    async with Simulator(port=0, interval=0.03, control_lab=True) as simulator:
        ...
        try:
            ...
            await hass.config_entries.options.async_configure(
                flow["flow_id"], user_input={"heater_w": 3000, "pump1_low_w": 350}
            )
            await hass.async_block_till_done()
            ...
            # A second options save updates ratings without reconnecting or rewriting old kWh.
            hass.config_entries.async_update_entry(
                entry, options={**entry.options, "energy_powers": {"heater_w": 2500}}
            )
            await hass.async_block_till_done()
